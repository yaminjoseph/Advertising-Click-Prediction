...
# Function 1: Coefficient of Variation Calculation
...
def num_biv_scat_hist_whue(dataset, col, date_col, hue_col):
    for col in col:
        # Scatterplot
        fig, ax = plt.subplots(figsize=(6, 4))
        sns.scatterplot(data=dataset, x=pd.to_datetime(date_col), y=col, hue=hue_col, ax=ax, palette='Set1')
        ax.grid(which="major", color=colors['gray'], alpha=0.4, linestyle='-', linewidth=0.5)
        ax.spines[['top', 'right']].set_visible(False)
